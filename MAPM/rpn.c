...
"       pi : 3.1415926 ...        @# : read stack  1=bottom, -1=top";
...
static void show_mathematica(M_APM x)
{
  ...
  *s++ = x->m_apm_sign < 0 ? '-' : '+'; // explicit sign
  ...
  if ((n = x->m_apm_exponent - n)) {    // Assume no overflow
    *s++ = n>0 ? '*' : (n=-n, '/');     // WolframAlpha ready
    *s++ = '1'; *s++ = '0'; *s++ = '^'; // 10^1 -> 10
    if (n>1) itoa(n, s, 10); else *--s = 0;
  }
  puts(buf);
  free(buf);
}
...
static void do_cmd(char *str)
{
  ...
  switch (c) {          // special commands or number
    ...
    case '@':           // array access, 1 = bottom, -1 = top
