...
"        p : y ^ x            ?[s|x] : show stack, no rounding\n"
...
"       pi : 3.1415926 ...        @# : array access,@1=bottom";
...
static void show_mathematica(M_APM x)
{
  ...
  char *s = M_to_data_str(buf, x, n, 0);
  if ((n = x->m_apm_exponent - n)) sprintf(s, "*10^%d", n);
  if (x->m_apm_sign < 0) putchar('-');
  puts(buf);
  free(buf);
}
/*******************************************************************/
...
static void do_func(struct opstr *cmd)
{
  ...
  switch(cmd->type) {           // apply function
    ...
    case MMA: cmd->func(x, top[-1]); return;
    case MiA: cmd->func(x, places); return;
  }
...
static void do_cmd(char *str)
{
  ...
  switch (c) {          // special commands or number
    ...
    case '@':           // array access on stack
      if (top == base + TOP) {puts("rpn: stack full"); return;}
      i = base36d(*s);
      if (++top != base + i) m_apm_copy(*top, base[i]);
      return;
    ...
    case '?':           // show top MAPM number
      ...
      switch(*s) {
        ...
        case 's': for(i = base-top; ++i; ) show_number(top[i]);
                  // fall thru
        case 'x': show_number(*top); return;
      }
      i = strtol(s, &s, 10);
...
int main(int argc, char **argv)
{
  ...
  for(int i=base-top; i++;) {           // print in reverse
    m_apm_iround(top[i], places);
    show_number(top[i]);
  }
  return 0;
}
